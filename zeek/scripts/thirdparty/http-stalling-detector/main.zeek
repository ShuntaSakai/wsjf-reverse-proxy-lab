...
export {
	...
	## --------------------------------------------------------------------------
    ## 遅延攻撃（HTTP Stalling / Slowloris）検知の閾値設定
    ##
    ## [too_much_client_delay]
    ##   リクエスト開始から「遅延」と判定するまでの時間（兼チェック間隔）。
    ##   サーバー側が短時間で接続を切断してしまう場合は、この値を短く（例: 2secs）設定します。
    ##
    ## [requests_threshold]
    ##   検知を発火させる「同時滞留リクエスト数」の閾値。
    ##   ターゲットのサーバー性能に応じて調整が必要です：
    ##     - マルチスレッドサーバー:
    ##       大量の接続を受け入れるため、デフォルト値 (40.0) で即座に検知可能です。
    ##     - シングルスレッドサーバー:
    ##       同時に受け入れる接続数が少ないため、閾値をその接続数以下
    ##       （例: 7.0）に下げないと検知されません。
    ## --------------------------------------------------------------------------
